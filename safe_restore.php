...
// 1. Ambil isi aslinya (Pure dari Framework Cache)
$c = file_get_contents($compiledPath);

// 2. Siapkan Otak Sehat Global
$brain = <<<'EOD'
<script>
window.zakatApp = () => ({
    ...
    carouselIndex: 0,
    carouselImages: [
        '<?php echo asset('images/beranda_annur_new.jpg'); ?>',
        '<?php echo asset('images/dokumentasi_1.jpg'); ?>'
    ],
    async loadChartJs() {
...
// 3. Masukkan script ke Head
$headMarker = "</head>";
$c = str_replace($headMarker, $brain . "\n" . $headMarker, $c);

// 4. Ubah x-data agar manggil fungsi global
$c = str_replace('x-data="zakatApp"', 'x-data="zakatApp()"', $c);

file_put_contents($targetPath, $c);
echo "SAFE RESTORE SUCCESS. 100% PHP SYNCED.\n";
