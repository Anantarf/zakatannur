<x-guest-layout>
    ...
    <div class="text-center mb-6">
        <h2 class="text-2xl font-bold tracking-[-0.01em] text-slate-900">Masuk ke Sistem</h2>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="mt-4" x-data="{ showPass: false }">
            ...
            <div class="relative mt-1">
                <input id="password" :type="showPass ? 'text' : 'password'" name="password" required autocomplete="current-password" class="block w-full rounded-md border-slate-200 shadow-sm focus:border-brand-500 focus:ring-brand-500 pr-12">
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox" class="rounded border-slate-200 text-brand-600 shadow-sm focus:ring-brand-500" name="remember">
                <span class="ml-2 text-sm text-slate-600">{{ __('Ingat Saya') }}</span>
            </label>
        </div>
