    ...
    <div class="ui-modal-box" x-show="openLogin" @click.away="openLogin = false"
        ...
        <div class="px-6 py-8 sm:p-10">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5" x-init="$watch('openLogin', value => { if(value) setTimeout(() => $refs.username.focus(), 100) })">
                ...
                <button type="submit" class="flex w-full justify-center rounded-xl bg-brand-600 px-4 py-3.5 text-sm font-bold text-white shadow-md hover:bg-brand-700 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 transition-all">
                    Masuk Sekarang
                </button>
            </form>
        </div>
