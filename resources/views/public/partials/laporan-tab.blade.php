<div x-show="activeTab === 'laporan'"
    ...
    <section class="public-report-shell">
        <div class="flex items-center justify-between border-b border-[#d7e5df] pb-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p
                        x-show="latestTransactionAt"
                        class="mt-1 text-[11px] font-medium text-slate-500"
                        x-text="latestTransactionAgeLabel"
                    >@if (!empty($summaryData['latest_transaction_at_wib']))Transaksi terakhir: {{ \Carbon\Carbon::parse($summaryData['latest_transaction_at_wib'], config('zakat.timezone'))->locale('id')->diffForHumans() }}@endif</p>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-2.5">
                    ...
                    <article class="public-report-mobile-row">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <span class="mt-1 inline-flex public-pill public-pill-sky text-[11px] tabular-nums" x-text="formatJiwa(item.total_jiwa)"></span>
                            </div>
                        </div>
                ...
                <article class="public-report-mobile-total">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <span class="public-pill public-pill-sky text-[11px] tabular-nums" x-text="formatJiwa(totals.total_jiwa)"></span>
                    </div>
        ...
        <div x-show="items.length > 0" class="mt-4 hidden overflow-hidden rounded-[1.1rem] border border-[#d7e5df] bg-[#f8fbf9]/88 md:block">
            <div class="grid grid-cols-[1.25fr_0.7fr_1fr_1fr] border-b border-[#d7e5df] bg-[#e8f0ec] px-5 py-3 text-[11px] font-bold uppercase tracking-[0.09em] text-slate-500">
                <span>Kategori</span>
            ...
            <div class="divide-y divide-[#d7e5df]">
                <template x-for="item in items" :key="'summary-' + item.category">
                    <article class="public-report-table-row grid-cols-[1.25fr_0.7fr_1fr_1fr]">
                        ...
                        <p class="text-right text-[0.98rem] font-semibold text-slate-700 tabular-nums" x-text="formatJiwaPlain(item.total_jiwa)"></p>
                ...
                <article class="grid grid-cols-[1.25fr_0.7fr_1fr_1fr] items-center gap-2.5 bg-[#e8f0ec] px-5 py-3.5">
                    <p class="text-sm font-bold uppercase tracking-[0.08em] text-slate-950">Total</p>
                    <p class="text-right text-[1rem] font-bold text-slate-950 tabular-nums" x-text="formatJiwaPlain(totals.total_jiwa)"></p>
