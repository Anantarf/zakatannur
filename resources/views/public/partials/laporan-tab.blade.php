<div x-show="activeTab === 'laporan'"
    ...
    <div class="mx-1 space-y-3 md:hidden">
        <section class="public-report-intro">
            ...
            <h2 class="mt-2 text-[1.4rem] font-semibold leading-tight text-slate-950">Lihat total utama per kategori zakat</h2>
        ...
        <template x-for="item in items" :key="'card-' + item.category">
            <article class="public-report-card">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <p class="text-[12px] font-medium tracking-[0.03em] text-slate-500">Kategori</p>
                        <h3 class="mt-1 text-[1.28rem] font-semibold leading-tight text-slate-950" x-text="formatCat(item.category)"></h3>
                    </div>
        ...
        <div x-show="items.length > 0" class="public-section public-section-muted">
            <p class="text-[12px] font-medium tracking-[0.03em] text-slate-500">Total penerimaan</p>
            <div class="mt-3 grid grid-cols-1 gap-3 text-[19px] tabular-nums">
                <span class="font-semibold text-slate-950" x-text="formatJiwa(totals.total_jiwa)"></span>
        ...
        <p x-show="items.length === 0" class="public-shell px-6 py-10 text-center text-sm font-bold text-slate-400">Belum ada data penerimaan masuk tahun ini.</p>
    </div>

    <div class="mx-1 hidden sm:mx-0 md:block">
        <div class="public-report-shell lg:p-5">
            <div class="flex items-end justify-between gap-4 border-b border-slate-200/75 px-2 pb-2.5">
                <div>
                    <p class="public-report-eyebrow">Ringkasan penerimaan</p>
                    <h2 class="mt-1 text-[1.1rem] font-semibold leading-tight text-slate-950">Kategori utama dalam satu panel ringkas</h2>
                </div>
            ...
            <div class="mt-2.5 grid grid-cols-[1.28fr_0.72fr_0.98fr_0.98fr] items-center gap-2.5 px-2 pb-1.5 text-[10px] font-medium tracking-[0.03em] text-slate-500">
                <span>Kategori</span>
            ...
            <div x-show="items.length > 0" class="space-y-2">
                <template x-for="item in items" :key="'summary-' + item.category">
                    <article class="public-report-row grid-cols-[1.28fr_0.72fr_0.98fr_0.98fr]">
                        <div class="min-w-0">
                            <p class="truncate text-[1.02rem] font-semibold text-slate-950" x-text="formatCat(item.category)"></p>
                        </div>
            ...
            <p x-show="items.length === 0" class="px-8 py-12 text-center text-base font-medium text-slate-400">Belum ada data penerimaan masuk tahun ini.</p>
        </div>
    </div>
