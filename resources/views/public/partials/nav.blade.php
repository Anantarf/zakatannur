<nav class="sticky top-2 z-[100] px-3 sm:px-4">
    <div class="mx-auto max-w-7xl rounded-[1.5rem] border border-white/[0.85] bg-white/[0.85] px-3 py-2.5 shadow-lg shadow-emerald-900/5 ring-1 ring-emerald-900/5 backdrop-blur-xl relative sm:px-4">
        <div class="grid grid-cols-[minmax(132px,1fr)_auto_minmax(132px,1fr)] items-center gap-2 sm:grid-cols-[minmax(180px,1fr)_auto_minmax(180px,1fr)] sm:gap-3">
            ...
            <div class="hidden lg:flex items-center gap-1 rounded-xl bg-emerald-950/[0.035] p-1 shrink-0">
                <button @click="activeTab = 'beranda'"
                    :class="activeTab === 'beranda' ? 'public-tab-chip-active' : 'public-tab-chip-inactive'"
                    class="public-tab-chip">
                    BERANDA
                </button>
                <button @click="activeTab = 'laporan'"
                    :class="activeTab === 'laporan' ? 'public-tab-chip-active' : 'public-tab-chip-inactive'"
                    class="public-tab-chip"
                    title="Laporan Penerimaan Zakat">
                    LAPORAN PENERIMAAN ZAKAT
                </button>
                <button @click="activeTab = 'grafik'"
                    :class="activeTab === 'grafik' ? 'public-tab-chip-active' : 'public-tab-chip-inactive'"
                    class="public-tab-chip"
                    title="Grafik Penerimaan Zakat">
        ...
        <div class="mt-2 flex items-center justify-center gap-1 border-t border-emerald-900/5 pt-2 lg:hidden">
            <button @click="activeTab = 'beranda'"
                :class="activeTab === 'beranda' ? 'public-tab-chip-mobile-active' : 'public-tab-chip-mobile-inactive'"
                class="public-tab-chip-mobile">
                BERANDA
            </button>
            <button @click="activeTab = 'laporan'"
                :class="activeTab === 'laporan' ? 'public-tab-chip-mobile-active' : 'public-tab-chip-mobile-inactive'"
                class="public-tab-chip-mobile">
                LAPORAN
            </button>
            <button @click="activeTab = 'grafik'"
                :class="activeTab === 'grafik' ? 'public-tab-chip-mobile-active' : 'public-tab-chip-mobile-inactive'"
                class="public-tab-chip-mobile">
                GRAFIK
            </button>
        </div>
