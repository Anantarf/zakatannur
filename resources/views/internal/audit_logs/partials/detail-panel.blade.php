<div x-data="{ open: false }">
    ...
    <div x-show="open" x-cloak x-transition class="mt-4 p-5 bg-slate-900 rounded-xl overflow-x-auto shadow-2xl border border-slate-700">
        @if(in_array($log->action, ['Updated.Transaction', 'Created.Transaction']))
            <div class="mb-4 pb-4 border-b border-slate-700/50">
                <div class="flex items-center justify-between mb-3">
                    <h4 class="text-xs font-semibold text-brand-400 uppercase tracking-[0.08em]">Ringkasan Transaksi</h4>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div class="bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
                        <div class="text-[10px] text-slate-500 uppercase font-semibold mb-1">Tambah</div>
                        <div class="text-lg font-bold text-brand-500">{{ $log->metadata['summary']['added'] ?? 0 }}</div>
                    </div>
                    <div class="bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
                        <div class="text-[10px] text-slate-500 uppercase font-semibold mb-1">Update</div>
                        <div class="text-lg font-bold text-amber-500">{{ $log->metadata['summary']['updated'] ?? 0 }}</div>
                    </div>
                    <div class="bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
                        <div class="text-[10px] text-slate-500 uppercase font-semibold mb-1">Hapus</div>
                        <div class="text-lg font-bold text-red-500">{{ $log->metadata['summary']['removed'] ?? 0 }}</div>
                    </div>
                </div>
            </div>
                ...
                @if(($log->metadata['totals']['old']['uang'] ?? 0) != ($log->metadata['totals']['new']['uang'] ?? 0))
                    <div class="flex items-center justify-between text-xs">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-brand-400 font-bold">{{ \App\Support\Format::rupiah((int)($log->metadata['totals']['new']['uang'] ?? 0)) }}</span>
                        </div>
                    </div>
                @endif
                @if(($log->metadata['totals']['old']['beras'] ?? 0) != ($log->metadata['totals']['new']['beras'] ?? 0))
                    <div class="flex items-center justify-between text-xs">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-brand-400 font-bold">{{ \App\Support\Format::kg((float)($log->metadata['totals']['new']['beras'] ?? 0)) }}</span>
                        </div>
                    </div>
                @endif
            </div>
        @elseif($log->action === 'system.bulk_transaction_cleanup')
            <div class="p-3 bg-slate-900 rounded-lg border border-slate-700 shadow-inner">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="text-[10px] font-semibold uppercase tracking-[0.08em] text-amber-400">
                        Pembersihan Sistem
                    </span>
                </div>
                <div class="space-y-1">
        ...
        @elseif(in_array($log->action, ['muzakki.deleted', 'muzakki.restored']))
            <div class="p-3 bg-{{ $log->action === 'muzakki.restored' ? 'emerald' : 'rose' }}-900/20 rounded-lg border border-{{ $log->action === 'muzakki.restored' ? 'emerald' : 'rose' }}-800/30">
                <div class="flex items-center gap-2 mb-2">
                    ...
                    <span class="text-xs font-semibold uppercase tracking-[0.08em] text-{{ $log->action === 'muzakki.restored' ? 'emerald' : 'rose' }}-400">
        ...
        @elseif(in_array($log->action, ['user.created', 'user.updated']))
            <div class="space-y-2">
                <h4 class="text-[10px] font-semibold text-amber-400 uppercase tracking-[0.08em] border-b border-amber-900/30 pb-1 flex items-center gap-2">
                ...
                <div class="grid grid-cols-1 gap-1 text-[11px]">
                    <div class="flex justify-between items-center bg-slate-800/20 p-2 rounded">
                        ...
                        <span class="text-slate-200 font-bold">{{ $log->metadata['name'] ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between items-center bg-slate-800/20 p-2 rounded">
        ...
        @elseif($log->action === 'transaction.sync_remove_item')
            <div class="space-y-2">
                <h4 class="text-[10px] font-semibold text-red-400 uppercase tracking-[0.08em] mb-2 border-b border-red-900/30 pb-1">Item Dihapus</h4>
                <div class="grid grid-cols-1 gap-1 text-[11px]">
            ...
            <div class="space-y-3">
                @foreach($log->metadata as $key => $val)
                    <div class="bg-slate-800/20 p-2 rounded border border-slate-700/30">
                        <span class="block text-[10px] text-slate-500 uppercase font-semibold mb-1 tracking-[0.08em]">{{ Str::headline($key) }}</span>
                        @if(is_array($val))
