        ...
        @foreach ($items as $t)
            <article class="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <span class="inline-flex rounded-md bg-blue-50 px-2 py-1 font-mono text-[11px] font-semibold text-blue-600">{{ $t->no_transaksi }}</span>
                        <div class="mt-2 text-sm font-semibold leading-tight text-gray-800">{{ $t->pembayar_nama }}</div>
                ...
                <div class="mt-3 grid grid-cols-1 gap-3 rounded-xl bg-gray-50 px-3 py-3">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <div class="text-right">
                            @if($t->total_uang > 0)
                                <div class="flex items-center justify-end gap-1.5">
                                    <div class="text-sm font-bold text-gray-800">{{ $t->total_uang_display }}</div>
                                    @if($t->has_transfer)
                            ...
                            @endif
                            @if($t->total_beras > 0)
                                <div class="mt-1 text-sm font-bold text-gray-800">{{ $t->total_beras_display }}</div>
                            @endif
                        </div>
                    </div>
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <div class="text-right">
                            <div class="text-sm font-medium text-gray-700">{{ $t->petugas?->name ?? '-' }}</div>
                            <span class="mt-1 inline-flex items-center justify-center rounded px-2 py-0.5 text-[10px] font-bold uppercase bg-emerald-50 text-emerald-700 border border-emerald-100 whitespace-nowrap leading-tight text-center">
                                {{ $t->shift_label }}
                            </span>
                        </div>
                    </div>
