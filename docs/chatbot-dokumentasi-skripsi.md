...
### 10.24 Bug: `ask_zakat_mal_nishab`/`ask_zakat_mal_definition`/`ask_zakat_mal_example` selalu menjawab entri KB generik, mengabaikan aset spesifik yang disebut user

**Gejala**: dilaporkan langsung dari chat production - pertanyaan *"nishab zakat penghasilan berapa"* dijawab penjelasan generik nisab/haul (85 gram emas, 653 kg gabah, 40 ekor kambing), bukan angka nisab penghasilan yang sebenarnya ditanyakan. Pesan ini tidak mengandung angka sama sekali, jadi bukan permintaan kalkulasi (lolos dari guard Bab 10.20) - ini pertanyaan informasi murni, tapi tetap salah sasaran.

**Akar masalah**: tiga intent (`ask_zakat_mal_nishab`, `ask_zakat_mal_definition`, `ask_zakat_mal_example`) di-*hardcode* memetakan **selalu** ke satu entri KB tetap ([ChatbotOrchestrator.php:174-179](../app/Services/Chatbot/ChatbotOrchestrator.php#L174-L179) - `nisab-dan-haul` untuk yang pertama, `zakat-mal` untuk dua lainnya) - tidak pernah mempertimbangkan apakah pesan menyebut aset spesifik (penghasilan, emas, tabungan, pertanian, dst.) yang punya entri KB sendiri jauh lebih relevan dan detail. Dikonfirmasi lewat `tinker`: *"Apa itu zakat penghasilan?"*, *"Apa itu zakat emas?"*, *"Pengertian zakat tabungan itu apa?"*, *"Definisi zakat pertanian gimana?"*, dan *"Kasih contoh zakat emas dong"* semuanya salah dijawab dari entri generik, bukan entri `zakat-penghasilan`/`zakat-emas-perak`/`zakat-tabungan`/`zakat-pertanian-perkebunan` yang jauh lebih tepat.

**Perbaikan** ([ChatbotActionDetector.php](../app/Services/Chatbot/ChatbotActionDetector.php)): menambah `$hasSpecificAssetTopic` - superset dari `$hasZakatMalSignal` (Bab 10.20) ditambah kata topik zakat mal lanjutan (pertanian, perkebunan, peternakan, properti, saham, investasi, reksadana, warisan, perdagangan, piutang, kendaraan, pensiun, hadiah, hibah, pesangon, campuran) - mencakup seluruh sub-topik yang punya entri KB tersendiri. Ketiga intent di atas kini digerbang `!$hasSpecificAssetTopic`: kalau pesan menyebut aset spesifik apa pun, jalur cepat dilewati dan pesan diteruskan ke AI+RAG, yang akan mengambil entri paling relevan (diverifikasi lewat `chatbot:eval-rag`). Pertanyaan benar-benar generik (*"Apa itu zakat mal?"*, *"Contoh zakat mal gimana?"*, *"Berapa nisab zakat mal?"*) tetap dapat jawaban cepat seperti biasa - tidak berubah.

**Verifikasi**: test regresi baru - `test_asset_specific_nishab_questions_are_not_hijacked_into_the_generic_definition` (3 kasus), `test_asset_specific_definition_and_example_questions_are_not_hijacked_into_the_generic_zakat_mal_entry` (6 kasus) - memastikan seluruhnya resolve ke `null` (lolos ke AI), plus dua test yang memastikan pertanyaan generik tetap fast-path seperti semula. Diverifikasi ulang lewat `chatbot:eval-rag` pasca-perbaikan: precision/recall/F1 tetap 1,0, tidak ada regresi retrieval.

**Pembelajaran metodologis**: kelas bug ini berbeda dari Bab 10.13-10.20 (yang soal salah *intent bucket* sama sekali) - di sini intent-nya sudah benar, tapi pemetaan ke KB di dalamnya terlalu kaku ("satu jawaban untuk semua"). Pola "jalur cepat X selalu jawab entri Y tetap" layak dicurigai sebagai kelas bug tersendiri di sistem berbasis KB + fast-path serupa - ditemukan justru setelah kelas bug "salah intent" (Bab 10.13-10.20) sudah dianggap tuntas, menunjukkan audit satu kelas bug tidak otomatis menutup kelas bug lain yang bersebelahan.

### 10.25 Bug: balasan lokasi/kontak masjid berisi data placeholder yang tidak pernah diganti data asli

**Gejala**: ditemukan lewat audit lanjutan (bukan laporan produksi) memicu pertanyaan langsung "apakah KB ada masalah lagi" - balasan untuk pertanyaan lokasi masjid ternyata berbunyi *"Masjid An-Nur berlokasi di **Jl. Contoh Alamat No. 123, Kelurahan Maju, Kecamatan Bersama, Kota Sejahtera**"*, dan balasan kontak menyebut *"WhatsApp/Telp: **0812-3456-7890 (Bapak Fulan)**"* - "Jl. Contoh Alamat", "Kelurahan Maju", "Kecamatan Bersama", "Kota Sejahtera", dan "Bapak Fulan" semuanya nama placeholder generik gaya template, bukan data asli, dan nomor teleponnya berpola angka berurutan (3456-7890) - ciri khas nomor contoh, bukan nomor sungguhan.

**Kenapa ini berbeda dari bug-bug sebelumnya**: seluruh bug Bab 10.1-10.24 soal *salah rute* atau *salah entri KB* - jawabannya salah sasaran tapi isinya sendiri tidak pernah bohong. Bug ini beda: kontennya sendiri **fiktif**, dan disampaikan dengan nada percaya diri seolah data resmi ke user sungguhan yang bertanya lokasi/kontak masjid untuk kebutuhan nyata (mis. mau datang bayar zakat langsung).

**Akar masalah**: teks ini didefinisikan langsung di kode ([ChatbotActionDetector.php](../app/Services/Chatbot/ChatbotActionDetector.php), intent `ask_location`/`ask_contact`) sebagai data contoh saat awal pengembangan, dan tidak ada satu pun sumber data lain di aplikasi (tidak ada pengaturan admin untuk alamat/kontak masjid) yang seharusnya menggantikannya - jadi bukan bug sinkronisasi seperti nisab (Bab 10.19), tapi data yang memang belum pernah diisi.

**Perbaikan** ([ChatbotActionDetector.php](../app/Services/Chatbot/ChatbotActionDetector.php), [OpenAiChatbotProvider.php](../app/Services/Chatbot/Providers/OpenAiChatbotProvider.php)): sesuai keputusan produk (tidak menampilkan alamat/nomor/nama panitia spesifik sama sekali), kedua balasan diganti jadi arahan umum ke panitia tanpa menyebut detail spesifik yang tidak terverifikasi. Instruksi system prompt (kedua bahasa) yang tadinya juga menyuruh LLM menyebutkan alamat/link peta yang sama diubah senada - eksplisit menyatakan tidak punya alamat terverifikasi untuk dibagikan.

**Verifikasi**: test regresi baru `test_location_and_contact_replies_do_not_contain_placeholder_data` memastikan balasan `ask_location`/`ask_contact` tidak lagi mengandung string placeholder ("Contoh Alamat", "Kelurahan Maju", "Bapak Fulan", "0812-3456-7890"). Regresi penuh tetap bersih.

**Pembelajaran metodologis**: kedua intent ini (`ask_location`, `ask_contact`) sebelumnya **tidak punya test sama sekali** - bug placeholder ini bisa saja bertahan tanpa batas waktu karena tidak ada assertion apa pun yang pernah memeriksa isi balasannya, hanya rutenya. Cakupan test yang memeriksa intent ter-*trigger* dengan benar tidak otomatis berarti isi jawabannya sudah diperiksa - dua hal yang terpisah.

**Regresi test suite penuh (Bab 10.24-10.25)**: `php artisan test` **338/338** (naik dari 326/326 di titik Bab 10.23 - 12 test baru, seluruhnya di `ChatbotActionDetectorTest`). Diverifikasi ulang lewat `chatbot:eval-rag` (P/R/F1 tetap 1,0) dan `chatbot:eval-behavior` (19/19, satu run awal sempat 18/19 pada skenario yang sama yang sebelumnya juga flaky di Bab 10.23 - dikonfirmasi nondeterminisme LLM, bukan regresi, lewat run ulang) pasca-perbaikan, 2026-08-08.
