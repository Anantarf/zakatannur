...
{
    public function exportDaily(string $date)
    {
        ...
        list($rowIdx, $totals) = $this->writeDailyRows($sheet, $transactions);
    ...
    public function exportYearly(int $year)
    {
        ...
        foreach ($summaryData as $row) {
            ...
            $grandTotal = $fitrahUang + $malUang + $infaqUang + $fidyahUang;
            ...
            $sheet->setCellValue('K' . $rowIdx, $grandTotal);
            $rowIdx++;
        }
    ...
    {
        ini_set('memory_limit', (string) config('zakat.export.yearly_memory_limit', '1024M'));
    ...
    private function setupDailySheet(Spreadsheet $spreadsheet, string $formattedDate)
    {
        ...
        foreach ([
            'A2' => 'No Transaksi', 'B2' => 'Nama', 'C2' => 'Shift', 'D2' => 'Fitrah',
            'F2' => 'Infaq', 'H2' => 'Fidyah', 'I2' => 'Mal', 'J2' => 'Jiwa',
            'K2' => 'Jumlah Zakat Fitrah', 'M2' => 'Transfer', 'N2' => 'Keterangan',
        ] as $cell => $value) {
            $sheet->setCellValue($cell, $value);
        }

        foreach (['A2:A3', 'B2:B3', 'C2:C3', 'D2:E2', 'F2:G2', 'H2:H3', 'I2:I3', 'J2:J3', 'K2:L2', 'M2:M3', 'N2:N3'] as $range) {
    ...
    private function writeDailyRows($sheet, $transactions): array
    {
        $rowIdx = 4;
        $totals = [
            'fitrahUang' => 0, 'fitrahBeras' => 0.0, 'infaqUang' => 0, 'infaqBeras' => 0.0,
            'fidyah' => 0, 'mal' => 0, 'jiwa' => 0, 'tf' => 0, 'cash' => 0,
        ];

        foreach ($transactions as $transaction) {
            $fitrahUang = (int) data_get($transaction, 'fitrah_uang');
            $fitrahBeras = (float) data_get($transaction, 'fitrah_beras');
            $infaqUang = (int) data_get($transaction, 'infaq_uang');
            $infaqBeras = (float) data_get($transaction, 'infaq_beras');
            $fidyah = (int) data_get($transaction, 'fidyah_uang');
            $mal = (int) data_get($transaction, 'mal_uang');
            $jiwa = (int) data_get($transaction, 'total_jiwa');
            $hasTransfer = (bool) data_get($transaction, 'has_transfer');

            $sheet->setCellValue('A' . $rowIdx, data_get($transaction, 'no_transaksi'));
            $sheet->setCellValue('B' . $rowIdx, data_get($transaction, 'pembayar_nama'));
            $sheet->setCellValue('C' . $rowIdx, ZakatTransaction::getShiftLabel(data_get($transaction, 'shift')));
            ...
            $sheet->setCellValue('M' . $rowIdx, $hasTransfer ? 'TF' : '');
            ...
            if ($hasTransfer) {
                ...
                foreach (['fitrah_has_tf' => ['D', 'K'], 'infaq_has_tf' => ['F'], 'fidyah_has_tf' => ['H'], 'mal_has_tf' => ['I']] as $field => $columns) {
                    if ((bool) data_get($transaction, $field)) {
                        foreach ($columns as $column) {
                            $sheet->getStyle($column . $rowIdx)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('A9D08E');
                        }
                    }
                }
            }

            $totals['tf'] += (int) data_get($transaction, 'tx_tf_uang');
            $totals['cash'] += (int) data_get($transaction, 'tx_cash_uang');
        ...
        if ($rowIdx > 4) {
            ...
            foreach (['D', 'F', 'H', 'I', 'K'] as $column) {
                $sheet->getStyle($column . '4:' . $column . $lastData)->getNumberFormat()->setFormatCode('#,##0');
            }
            foreach (['E', 'G', 'L'] as $column) {
                $sheet->getStyle($column . '4:' . $column . $lastData)->getNumberFormat()->setFormatCode('#,##0.00');
            }
        }
    ...
    private function writeDailyTotalRow($sheet, int $rowIdx, array $totals): void
    {
        ...
        foreach (['E', 'G', 'L'] as $column) {
            $sheet->getStyle($column . $rowIdx)->getNumberFormat()->setFormatCode('#,##0.00');
        }
    }
